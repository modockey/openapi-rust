    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
        ...
        context: &C) -> Result<IpGetResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<models::IpGet200Response>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(IpGetResponse::GetGlobalIPv
                    (body)
                )
            }
            500 => {
    ...
    async fn ip_post(
        &self,
        param_ip_get_request: Option<models::IpGetRequest>,
        context: &C) -> Result<IpPostResponse, ApiError>
    {
        ...
        let body = param_ip_get_request.map(|ref body| {
